...
// below mentioned packages are used for transforming the content 
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { oneLight } from 'react-syntax-highlighter/dist/cjs/styles/prism';// to give themes to the code block
...
const AIResponsePreview = ({content}) => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
        <div className="text-sm prose prose-slate dark:prose-invert max-w-none">{/* "prose" is a typography class of tailwind */}
            <ReactMarkdown 
            ...
            components={{
                ...
                a({children,href}) {
                    return <a href={href} className="text-base text-indigo-500 hover:underline">{children}</a>
                },
                table({children}) {
                    return (
        ...
        <div className="relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200">
            <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-gray-200">
                <div className="flex items-center space-x-2">
                    ...
                    <span className="text-base font-bold text-indigo-800 uppercase tracking-wide">
                        {language || 'Code'}
                    </span>
                </div>
                <button
                onClick={handleCopy}
                className="text-gray-500 cursor-pointer hover:text-gray-700 focus focus-outline-none relative group"
            ...
            <SyntaxHighlighter
                language={language}
                style={dracula}
                customStyle={{fontSize:13.5, margin: 0, padding: '1rem',background: '#23252b'}}
            >
                {code}
            </SyntaxHighlighter>
        </div>
